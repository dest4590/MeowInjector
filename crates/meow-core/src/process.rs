...
use windows_sys::Win32::System::Threading::{
    OpenProcess, PROCESS_DUP_HANDLE, PROCESS_QUERY_INFORMATION, PROCESS_QUERY_LIMITED_INFORMATION,
...
pub fn list_processes() -> Vec<(String, u32)> {
    vec![
        ("steam.exe".into(), 1234),
        ("game.exe".into(), 5678),
        ("notepad.exe".into(), 9012),
        ("explorer.exe".into(), 3456),
        ("chrome.exe".into(), 7890),
        ("discord.exe".into(), 1357),
        ("obsidian.exe".into(), 2468),
        ("vscode.exe".into(), 1111),
    ]
}
